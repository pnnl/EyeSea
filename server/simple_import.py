...
def importnxcode(dirr, outext = 'mp4', vcodec = 'libx264', vext = ['avi', 'mp4']):
    for f in os.listdir(dirr):
        if f.split('.')[-1] in vext:
            ...
            data = video.select().where(video.vid==video.insert(filename = outfile, description = 'Import of' + dirr, fps = 25, variable_framerate = False, uri = 'file://' + dirr + outfile).execute()).dicts().get()
            print data
